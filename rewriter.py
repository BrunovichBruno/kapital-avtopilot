...
ВАЖНО: если новость не про финансы, банки, инвестиции или экономику — верни {{"skip": true}}.
Публикуем только финансовый контент.
...
ВАЖНО: если материал не про финансы, банки или экономику — верни {{"skip": true}}.
...
def rewrite_article(title, body, api_key, model_name="gemini-3.6-flash", summary=""):
    ...
    for m in [model_name] + [x for x in FALLBACK_MODELS if x != model_name]:
        ...
        if result:
            # Проверка на skip
            if result.get("skip"):
                print("[rewriter] Gemini пометил как нерелевантное")
                return None
            result["style"] = style
